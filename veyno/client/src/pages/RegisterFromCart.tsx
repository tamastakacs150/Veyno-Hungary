...
export default function Register() {
    ...
    const cartToKeep = (cartFromState && Array.isArray(cartFromState) && cartFromState.length)
        ? cartFromState
        : (cart || []);
    const { register: doRegister, loginWithGoogle, user } = useAuth();

    if (user) {
        return <Navigate to="/checkout" replace state={{ from: "cart", cart: cartFromState }} />;
    }
    ...
    const submit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setErr("");
            sessionStorage.setItem("postVerifyRedirect", "checkout");
            sessionStorage.setItem("arrivedFromVerify", "1");
            try { localStorage.setItem("cart", JSON.stringify(cartToKeep)); } catch { }

            nav(`/verify-wait?email=${encodeURIComponent(form.email)}`, {
                replace: true,
